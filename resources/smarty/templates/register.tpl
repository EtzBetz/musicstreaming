            ...
            {else}
                <form action="{$configArr.urls.registering}" onsubmit="return validateForm()" method="post" class="column">
                    ...
                    <div class="column">
                        ...
                        <input class="register" placeholder="{$configArr.strings.email}" type="email" alt="E-Mail" id="email" name="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{ldelim}2,4{rdelim}$" required>
                    ...
                    <div class="column">
                        ...
                        <input class="register" placeholder="{$configArr.strings.username}" alt="Username" id="username" name="username" pattern="[a-zA-Z0-9]{ldelim}3,20{rdelim}" required>
                        <span class="warning">{$configArr.strings.enterValidAccountname}</span>
                    </div>
                    <div class="column">
                        {*<label for="password" class="">Password:</label>*}
                        <input class="register" placeholder="{$configArr.strings.password}" type="password" alt="Password" id="password" name="password" minlength="6" required>
                        <span class="warning">{$configArr.strings.passwordHint}</span>
                    </div>
                    <div class="column">
                        {*<label for="passwordRepeat" class="">Repeat Password:</label>*}
                        <input class="register" placeholder="{$configArr.strings.repeatPassword}" type="password" alt="Repeat Password" id="passwordRepeat" name="passwordRepeat" minlength="6" required>
                        <span class="warning">{$configArr.strings.passwordHint}</span>
                        <span class="warning">{$configArr.strings.passwordNotMatching}</span>
                    </div>
                    <div>
                        <button class="register" type="submit">{$configArr.strings.register}</button>
                    </div>
                </form>
            {/if}
        </div>
